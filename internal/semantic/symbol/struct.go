...
func (sds *StructDefSymbol) Equals(other Symbol) bool {
	if other.Type() == environment.ANY_OBJ || sds.Type() == environment.ANY_OBJ {
		// Wait, ANY_OBJ is returned by StructDefSymbol's Type().
		// We shouldn't automatically return true here if both are struct defs.
	}
