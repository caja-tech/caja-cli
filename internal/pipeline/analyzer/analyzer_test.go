...
func TestSemanticAnalysisBuiltins(t *testing.T) {
	tests := []testScenario{
		...
		// Systemic check: a Nullable value is rejected wherever a plain,
		// non-nullable value is required — NullableSymbol.Type()
		// deliberately forwards to its underlying type, so a plain .Type()
		// check alone would silently accept it too.
		{
