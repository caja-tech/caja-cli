...
func transpileStatement(stmt ast.Statement, ctx *transpileContext) (string, error) {
	...
	case *ast.LetStatement:
		...
		val, err := transpileExpression(s.Value, ctx, varType)
		if err != nil {
			return "", err
		}
...
func transpileExpression(expr ast.Expression, ctx *transpileContext, expectedType string) (string, error) {
	if expr == nil {
		return "", nil
	}
	a := ctx.analyzer
	...
	case *ast.CallExpression:
		if prop, ok := e.Function.(*ast.PropertyExpression); ok {
			if ident, ok := prop.Object.(*ast.Identifier); ok && builtinModules[ident.Value] {
				return transpileBuiltinCall(ident.Value, prop.Property.Value, e.Arguments, ctx)
			}
		}
